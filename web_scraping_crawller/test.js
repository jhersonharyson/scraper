...
const fs = require("fs");
const { JSDOM } = jsdom;
const html = fs.readFileSync('C:/Users/J.Haryson/Desktop/tcc/teste/scraper/web_scraping_crawller/public/index.html', 'utf8')
...
// const dom = new JSDOM(response.data, {runScripts: "dangerously", includeNodeLocations: true })
...
const findPropertyWithNames = [
  ...
  const buildCatalogWithMatchFontSize = (onePage, matchFontSize) => {
    ...
    for (const elements of onePage) {
      ...
      (matchFontSize + 2 > +elementFontSize) && (matchFontSize - 2 < +elementFontSize) && console.log('true')
  
      // if data is a organ catalog then
      if (
        (matchFontSize + 2 > +elementFontSize) && (matchFontSize - 2 < +elementFontSize) && 
        !!element.innerText &&
  ...
  const sendDataToServer = async (data) => {
    ...
    const settings = {
      ...
      try{
        ...
        // await fetch('http://backend-server:8080/scraper', { method: 'GET',} )
        // const fetchResponse = await fetch('http://localhost:8080/scraper', settings )
